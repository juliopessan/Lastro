...
- **A proposta renderizada mistura os dois com selo visual diferente.** `src/app/propostas/[id]/page.tsx` mostra os números do briefing dentro de um painel escuro ("ledger"). Pra dentro, o painel leva o selo verde de "medido, não estimado" e um aviso laranja de que os parágrafos abaixo foram escritos por IA: são duas checagens de quem vai enviar. Pra fora, no lugar do selo verde entra uma nota de estimativa, dizendo que valores e prazos podem ser ajustados em acordo com o cliente. O verde não pode acompanhar essa frase: "medido, não estimado" sobre um texto que diz "são estimados" seria um carimbo de verificado contradizendo o próprio conteúdo, que é exatamente o que esse sistema visual existe pra impedir.
- **O texto do cliente não tem cara de IA.** O prompt em `src/lib/ai.ts` proíbe travessão e hífen duplo no meio das frases, e troca jargão por palavra de cliente ("projeto" no lugar de MVP, "reunião de início" no lugar de kickoff). Nas telas, o marcador dos itens de escopo é ponto médio e não traço, porque uma coluna de traços entrega o documento sozinha.
